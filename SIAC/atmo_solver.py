...
class solving_atmo_paras(object):
    ...
    def _multi_grid_solver(self, ):
        ...
        for ii in order:
            ...
            if self.vza.ndim == 2:
                for i, parameter in enumerate(
                        [self.sza, self.vza, self.raa, self.aot_prior, self.tcwv_prior, self.tco3_prior, self.ele]):
                    self.control_variables[:, i, :, :] = self._grid_conversion(parameter, shape)

            elif self.vza.ndim == 3:
                for j in range(len(self.vza)):
                    for i, parameter in enumerate(
                            [self.sza, self.vza[j], self.raa[j], self.aot_prior, self.tcwv_prior, self.tco3_prior,
                             self.ele]):
            ...
            subs = [np.array_split(sub, self.num_blocks_y, axis=1) for sub in
                    np.array_split(self._coarse_num, self.num_blocks_x, axis=0)]
            ...
            psolve = optimize.minimize(self._cost, p0, jac=True, bounds=bounds, method='L-BFGS-B',
                                       options={'disp': False, 'maxcor': mc, 'gtol': 1e-02, 'ftol': ftol, 'maxfun': mf,
                                                'maxiter': mi})
            ...
            if ii != len(shapes) - 1:
                mask = (self.aot_prior <= 0) | (self.aot_prior > 2.) | (self.tcwv_prior <= self.bot_bounds[1]) | (
                            self.tcwv_prior > self.up_bounds[1])
                ...
                to_inv = np.nansum(
                    [sparse.diags((self.obs_unc[0]).ravel()), sparse.diags((self.prior_uncs[0] ** -2).ravel()),
                     self.gamma ** 2 * dtd], axis=0)
                aot_unc = (linalg.inv(to_inv).diagonal()) ** 0.5

                to_inv = np.nansum(
                    [sparse.diags((self.obs_unc[1]).ravel()), sparse.diags((self.prior_uncs[1] ** -2).ravel()),
                     self.gamma ** 2 * dtd], axis=0)
        ...
        self.tco3_prior, self.tco3_unc = self._grid_conversion(self.tco3_prior, shape), self._grid_conversion(
            self.tco3_unc, shape)
    ...
    def _obs_cost_test(self, p, is_full=True, do_unc=False):
        ...
        if is_full:
            ...
            if do_unc:
                comb_unc = np.nansum(
                    [self.band_weights[..., None] * (dH[:, :, i] ** 2) * (self.boa_unc ** -2) for i in range(2)],
                    axis=1)
                comb_unc[comb_unc == 0] = np.nan
                self.obs_unc = np.zeros((2,) + self.full_res)
                self.obs_unc[:] = np.nan
                self.obs_unc[:, self.Hx, self.Hy] = comb_unc
                subs = [np.array_split(sub, self.num_blocks_y, axis=2) for sub in
                        np.array_split(self.obs_unc, self.num_blocks_x, axis=1)]
