...
from SIAC.globals import emus_dir, mcd43_dir, global_dem, cams_dir, spec_m_dir
...
class solve_aerosol(object):
    ...
    def _create_base_map(self, ):
        ...
        if self.aoi is not None:
            if os.path.exists(self.aoi):
                try:
                    ...
                    subprocess.call(
                        ['gdaltindex', '-f', 'GeoJSON', '-t_srs', 'EPSG:4326', self.toa_dir + '/AOI.json', self.aoi])
                except:
                    try:
        ...
        if not os.path.exists(self.toa_dir + '/AOI.json'):
            ...
            subprocess.call(
                ['gdaltindex', '-f', 'GeoJSON', '-t_srs', 'EPSG:4326', self.toa_dir + '/AOI.json', self.toa_bands[0]])
    ...
    def _create_band_gs(self, ):
        ...
        for band in self.toa_bands:
            g = gdal.Warp('', str(band), format='MEM', xRes=self.pixel_res, yRes=self.pixel_res,
        ...
        for i in range(3):
            if priors[i] is None:
                priors[i] = self.cams_dir + '/'.join([datetime.strftime(self.obs_time, '%Y_%m_%d'), \
                                                      datetime.strftime(self.obs_time, '%Y_%m_%d') + '_%s.tif' %
                                                      cams_names[i]])
                use_cams[i] = True
    ...
    def _read_MCD43(self, fnames):
        def warp_data(fname, aoi, xRes, yRes):
            g = gdal.Warp('', fname, format='MEM', srcNodata=32767, dstNodata=0, \
                          cutlineDSName=aoi, xRes=xRes, yRes=yRes, cropToCutline=True,
                          resampleAlg=0)  # weird adaptation for gdal 2.3, this should be a bug in gdal 2.3
                ...
                for day in days:
                    for band in self.boa_bands:
                        fname = self.mcd43_dir + '/'.join(
                            [day.strftime('%Y-%m-%d'), temp % (day.strftime('%Y%j'), band)])
                        fnames.append(fname)
        else:
            fnames = []
            for temp in [da_temp, qa_temp]:
                for band in self.boa_bands:
                    fname = self.MCD43_dir + '/'.join(
                        [datetime.strftime(self.obs_time, '%Y-%m-%d'), temp % (doy, band)])
                    fnames.append(fname)
        ...
        if temporal_filling:
            ...
            def smooth(da_w):
                ...
                if (da.shape[-1] == 0) | (w.shape[-1] == 0):
                    return da[mid], w[mid]
                data = \
                np.array(smoothn(da, s=10., smoothOrder=1., axis=0, TolZ=0.001, verbose=False, isrobust=True, W=w))[
                    [0, 3],]
    ...
    def _get_index(self, mg, hg):
        ...
        h_xgeo, h_ygeo, _ = np.array(osr.CoordinateTransformation(m_proj, \
                                                                  h_proj).TransformPoints(
            list(zip(xgeo[temp_data], ygeo[temp_data])))).T
    ...
    def _load_xa_xb_xc_emus(self, ):
        xap_emu = glob(self.emus_dir + '/isotropic_%s_emulators_correction_xap_%s.pkl' % (self.sensor, self.satellite))[
            0]
        xbp_emu = glob(self.emus_dir + '/isotropic_%s_emulators_correction_xbp_%s.pkl' % (self.sensor, self.satellite))[
            0]
        xcp_emu = glob(self.emus_dir + '/isotropic_%s_emulators_correction_xcp_%s.pkl' % (self.sensor, self.satellite))[
            0]
    ...
    def _get_convolved_toa(self, ):
        ...
        xgaus = np.exp(
            -2. * (np.pi ** 2) * (self.psf_xstd ** 2) * ((0.5 * np.arange(self.full_res[0]) / self.full_res[0]) ** 2))
        ygaus = np.exp(
            -2. * (np.pi ** 2) * (self.psf_ystd ** 2) * ((0.5 * np.arange(self.full_res[1]) / self.full_res[1]) ** 2))
...
def test_s2():
    ...
    aero = solve_aerosol(sensor_sat, toa_bands, band_wv, band_index, view_angles, sun_angles, obs_time, cloud_mask,
                         gamma=10.)
    aero._solving()
    return aero
...
def test_l8():
    ...
    aero = solve_aerosol(sensor_sat, toa_bands, band_wv, band_index, view_angles, sun_angles, obs_time, cloud_mask,
                         gamma=10., ref_scale=scale, ref_off=off)
    aero._solving()
    return aero
...
def test_modis():
    ...
    for i in range(7):
        ...
        ds = driver.Create(toa_dir + band_name, 2400, 2400, 1, gdal.GDT_Float32,
                           options=["TILED=YES", "COMPRESS=DEFLATE"])
        ...
        ds.FlushCache()
        ds = None
    toa_bands = [toa_dir + '/MODIS_' + tile + obs_time.strftime('_%Y%m%d_%H%M_') + 'band%d.tif' % i for i in
                 [3, 4, 1, 2, 6, 7]]
    ...
    aero = solve_aerosol(sensor_sat, toa_bands, band_wv, band_index, view_angles, sun_angles, obs_time, cloud_mask,
                         gamma=gamma, emus_dir=emus_dir, ref_scale=scale, ref_off=off, aero_res=3000)
    aero._solving()
    return aero
