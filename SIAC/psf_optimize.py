...
class psf_optimize(object):
    ...
    def _preprocess(self, ):
        ...
        self.bad_pixs = cloud_dilation((self.high_img < 0.0001) | self.cloud | (self.high_img >= 1),
                                       iteration=int(size / 2))
        # xstd, ystd = 29.75, 39
    ...
    def gaussian(self, xstd, ystd, angle, norm=True):
        ...
        cgaus = r_gaus[int(center[0] - win / 2): int(center[0] + win / 2),
                int(center[1] - win / 2):int(center[1] + win / 2)]
        if norm:
