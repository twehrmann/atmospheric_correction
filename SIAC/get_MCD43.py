...
try:
    ...
    logger.error(
        'Environment variables Earthdata_user and Earthdata_pass for accessing NASA Earthdata are not set, and trying to read from file or input.')
...
if os.path.exists(file_path + '/data/.earthdata_auth'):
    try:
        ...
        logger.error(
            'Please provide NASA Earthdata username and password for downloading MCD43 data, which can be applied here: https://urs.earthdata.nasa.gov.')
        username = input('Username for NASA Earthdata: ')
        password = getpass.getpass('Password for NASA Earthdata: ')
...
def find_files(aoi, obs_time, temporal_window=16):
    logger.info('find files {},{},{} ...'.format(aoi, obs_time, temporal_window))
...
def get_one_tile(tile_date):
    ...
    tile, date = tile_date
    for j in range(100):
        logger.info('Scanning MCD ({}) product {}... {}'.format(tile_date, base + date, j))
    ...
    if fname:
        ...
        raise Exception("Could not find MCD data for {}".format(tile_date))


def downloader(url_fname):
    logger.info('Downloading {}...'.format(url_fname))
