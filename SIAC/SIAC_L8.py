...
def SIAC_L8(l8_dir, send_back=False, mcd43=home + '/MCD43/', vrt_dir=home + '/MCD43_VRT/', aoi=None):
    ...
    if len(glob(file_path + '/emus/' + 'isotropic_OLI_emulators_*_x?p_L8.pkl')) < 6:
        url = 'https://s3-eu-west-1.amazonaws.com/public.eoss.cloud/data/emus/'
...
def exe():
    parser = argparse.ArgumentParser(description='Landsat OLI Atmospheric Correction Excutable')
    parser.add_argument('-f', "--file_path",      help='OLI data file path', required=True)
    ...
    SIAC_L8(l8_dir=args.file_path, mcd43=args.MCD43_file_dir, vrt_dir=args.vrt_dir, aoi=args.aoi)
