...
def parse_xml(meta_file, example_file, sun_ang_name):
    ...
    dst_ds = gdal.GetDriverByName('GTiff').Create(sun_ang_name, 23, 23, 2, gdal.GDT_Int16,
...
def get_angle(view_ang_name_gml, vaa, vza, band_dict):
    ...
    try:
        ...
        dst_ds = gdal.GetDriverByName('GTiff').Create(view_ang_name, x_size, y_size, 2, gdal.GDT_Int16,
                                                      options=["TILED=YES", "COMPRESS=DEFLATE"])
...
def resample_s2_angles(metafile):
    ...
    if procs < 1:
        raise MemoryError('At least 500MB ram is needed. ({})'.format(procs))
