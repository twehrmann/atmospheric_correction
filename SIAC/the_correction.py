...
from SIAC.globals import emus_dir, global_dem, cams_dir
...
class atmospheric_correction(object):
    ...
    def _fill_nan(self, ):
        ...
        self._saa, self._sza, self._ele, self._aot, self._tcwv, self._tco3, self._aot_unc, self._tcwv_unc, self._tco3_unc = \
            parmap(fill_nan,
                   [self._saa, self._sza, self._ele, self._aot, self._tcwv, self._tco3, self._aot_unc, self._tcwv_unc,
                    self._tco3_unc])
    ...
    def _get_xps(self, ):
        ...
        del self._aot;
        del self._tcwv;
        del self._tco3  # ; del self._aot_unc; del self._tcwv_unc; del self._tco3_unc
    ...
    def _do_chunk(self, ind):
        ...
        for i in range(self._chunks):
            ...
            _g = gdal.Warp('', toa_g, format='MEM', outputBounds=[xmin, geo[3] + y_size * geo[5], xmax, geo[3]],
                           resampleAlg=0)
            xRes = yRes = int(geo[1])
            ...
            for unc_g in [self._aot_unc, self._tcwv_unc, self._tco3_unc]:
                ...
                tmp.append(unc)
            aot_unc, tcwv_unc, tco3_unc = tmp;
            del tmp
            unc = np.sqrt(
                aot_dH ** 2 * aot_unc ** 2 + tcwv_dH ** 2 * tcwv_unc ** 2 + tco3_dH ** 2 * tco3_unc ** 2 + toa_dH ** 2 * 0.015 ** 2)
            del aot_unc;
            del tcwv_unc;
            del tco3_unc
            boas.append(boa)
    ...
    def _save_rgb(self, rgba_array, outputFileName, projection, geotransform):
        ...
        if os.path.exists(outputFileName):
            os.remove(outputFileName)
        dst_ds = gdal.GetDriverByName('GTiff').Create(outputFileName, ny, nx, 4, gdal.GDT_Byte,
    ...
    def _compose_rgb(self, ):
        ...
        rgba_array = np.clip([self.boa_rgb[0] * self.rgb_scale * 255, self.boa_rgb[1] * self.rgb_scale * 255, \
                              self.boa_rgb[2] * self.rgb_scale * 255, alpha * self.rgb_scale * 255], 0, 255).astype(
            np.uint8)
...
def test_s2():
    ...
    atmo = atmospheric_correction(sensor_sat, toa_bands, band_index, view_angles, sun_angles, aot=aot,
...
def test_modis():
    ...
    rgb = [toa_bands[2], toa_bands[1], toa_bands[0]]
    atmo = atmospheric_correction(sensor_sat, toa_bands, band_index, view_angles, sun_angles, aot=aot,
                                  cloud_mask=cloud_mask, ref_scale=scale, ref_off=off, \
                                  tcwv=tcwv, tco3=tco3, aot_unc=aot_unc, tcwv_unc=tcwv_unc, tco3_unc=tco3_unc, rgb=rgb,
                                  emus_dir=emus_dir, block_size=3000)
    ret = atmo._doing_correction()
    return ret, atmo
